...
<%--弹窗--%>
<form id="layer" action="/user/edituser" method="post">
    <div class="title">
        <h2>修改角色</h2>
    </div>
    ...
    <c:forEach items="${requestScope.pager.rows}" var="u">
        <tr id="tr">
            ...
            <td style="width: 400px">
                <a href="javascript:;" onclick="showLayer_edit('layer', 400, 600,'${u.id}','${u.username}',
                        '${u.sex}','${u.birth}')"> 修 改</a>
            </td>
        </tr>
    </c:forEach>
...
<br/><br/>
<%
    out.println("<div class=\"div1\"><a href=\"/user/findGByPager?page=1&size="+ size +"\">首页</a>&nbsp;");
    if (pages > 1) {
        out.println("<a href=\"/user/findGByPager?page="+ (pages-1) +"&size="+ size +"\">上一页</a>&nbsp;");
    }
    out.println("第"+ pages +"/"+ ((total+size-1)/size) +"页&nbsp;&nbsp;&nbsp;");
    out.println("共"+ total +"条数据&nbsp;&nbsp;&nbsp;");
    if (pages < ((total+size-1)/size)) {
        out.println("<a href=\"/user/findGByPager?page="+ (pages+1) +"&size="+ size +"\">下一页</a>&nbsp;");
    }
    out.println("<a href=\"/user/findGByPager?page="+ ((total+size-1)/size) +"&size="+ size +"\">尾页</a>&nbsp;</div>");
%>
