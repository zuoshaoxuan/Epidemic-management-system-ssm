...
%>
    <form id="layer_edit" action="/caip/orderCaip" method="post">
        <div class="title">
            <h2>确 认 订 单</h2>
        </div>
        <input type="text" name="id" id="id" hidden/>
        <p><label> 用 户 名: </label><input type="text" name="username" value="${sessionScope.x}"/></p>
        <p><label> 菜 单 表: </label><input type="text" name="temp" id="caip"/></p>
...
<script language="javascript">
    ...
    <table border="1px" cellpadding="0" cellspacing="0">
        <tr><th>菜品名称</th><th>主要成分</th><th>选择</th></tr>
    ...
    <%
        ...
        out.println("<a href=\"/caip/findcaipByPager?page=1&size="+ size +"\">首页</a>&nbsp;");
        if (pages > 1) {
            out.println("<a href=\"/caip/findcaipByPager?page="+ (pages-1) +"&size="+ size +"\">上一页</a>&nbsp;");
        }
        out.println("第"+ pages +"/"+ ((total+size-1)/size) +"页&nbsp;&nbsp;&nbsp;");
        out.println("共"+ total +"条数据&nbsp;&nbsp;&nbsp;");
        if (pages < ((total+size-1)/size)) {
            out.println("<a href=\"/caip/findcaipByPager?page="+ (pages+1) +"&size="+ size +"\">下一页</a>&nbsp;");
        }
        out.println("<a href=\"/caip/findcaipByPager?page="+ ((total+size-1)/size) +"&size="+ size +"\">尾页</a>&nbsp;</div>");
    %>
    <br>
</body>
</html>
