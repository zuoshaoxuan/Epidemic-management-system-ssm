...
<div class="div">
    <form action="/caip/FuzzyQueryBycpname?page=1&size=7" method="post">
        <input type="search" placeholder="输入关键字" name="cpname" required/> <input type="submit" value=" 搜 索 ">
    </form>
...
<br/><br/>
<%
    out.println("<div class=\"div1\"><a href=\"/caip/findAllCaipByPager?page=1&size="+ size +"\">首页</a>&nbsp;");
    if (pages > 1) {
        out.println("<a href=\"/caip/findAllCaipByPager?page="+ (pages-1) +"&size="+ size +"\">上一页</a>&nbsp;");
    }
    out.println("第"+ pages +"/"+ ((total+size-1)/size) +"页&nbsp;&nbsp;&nbsp;");
    out.println("共"+ total +"条数据&nbsp;&nbsp;&nbsp;");
    if (pages < ((total+size-1)/size)) {
        out.println("<a href=\"/caip/findAllCaipByPager?page="+ (pages+1) +"&size="+ size +"\">下一页</a>&nbsp;");
    }
    out.println("<a href=\"/caip/findAllCaipByPager?page="+ ((total+size-1)/size) +"&size="+ size +"\">尾页</a>&nbsp;</div>");
%>
